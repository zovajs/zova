...
export async function generateFileComponent(options, globFile, controllerInfo) {
    ...
    if (hasModels)
        _contentImportTypeZova.push('DefineModelOptions', 'TypePropUpdateFromModel', 'TypePropValueFromModel');
    if (hasProps)
        _contentImportTypeZova.push('TypeControllerInnerProps');
    if (_contentImportTypeZova.length > 0) {
        contentImports.push(`import type { ${_contentImportTypeZova.join(', ')} } from 'zova';`);
    }
    const _contentImportTypeController = [];
    if (hasModels)
        _contentImportTypeController.push(nameModels);
    if (hasProps)
        _contentImportTypeController.push(nameProps);
...
} &
{
  [KEY in keyof ${nameModels}${genericArguments} as TypePropUpdateFromModel<KEY>]: (value: ${nameModels}${genericArguments}[KEY]) => void;
    ...
    // Controller
    const contentControllerInterfaceMethods = [];
    if (hasProps) {
